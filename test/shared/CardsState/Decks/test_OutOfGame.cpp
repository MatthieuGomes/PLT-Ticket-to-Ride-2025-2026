...
#include "../../src/shared/cardsState/OutOfGame.h"
...
TEST(Default)
{
    ...
    }
    {
        ...
        OutOfGame<DestinationCard> deck;
        CHECK_EQ(deck.countCards(), 0);
        CHECK(deck.cards.empty());
...
{
    {
        ...
        OutOfGame<WagonCard> deck(cardsVec);
    ...
    }
    {
        ...
        OutOfGame<DestinationCard> deck(cardsVec);
    ...
    }
    {
        ...
        OutOfGame<WagonCard> deck(emptyVec);
        CHECK_EQ(deck.countCards(), 0);
        CHECK(deck.cards.empty());
...
TEST(shuffle)
{
    ...
    }
    {
        ...
        std::vector<std::shared_ptr<WagonCard>> shuffledCardsVec = OutOfGame<WagonCard>::shuffle(cardsVec);
...
{
    {
        ...
        WagonCard card2(cardArgs[1]);
        std::vector<std::shared_ptr<WagonCard>> cardsVec = std::vector<std::shared_ptr<WagonCard>>{std::make_shared<WagonCard>(card1), std::make_shared<WagonCard>(card2)};
        auto originOutOfGame = std::make_shared<OutOfGame<WagonCard>>(cardsVec);
        auto destinationOutOfGame = std::make_shared<OutOfGame<WagonCard>>();

        OutOfGame<WagonCard>::moveCardTo(originOutOfGame, destinationOutOfGame, 0);

        CHECK_EQ(originOutOfGame->countCards(), 1);
        CHECK_EQ(destinationOutOfGame->countCards(), 1);

        CHECK_EQ(originOutOfGame->cards[0]->color, ColorCard::BLUE);
        CHECK_EQ(destinationOutOfGame->cards[0]->color, ColorCard::RED);
        ANN_END("moveCardTo")
    }
    {
        ...
        CHECK_NTHROW(OutOfGame<WagonCard>::moveCardTo(nullptr, destinationOutOfGame, 1)); 
        CHECK_NTHROW(OutOfGame<WagonCard>::moveCardTo(originOutOfGame, nullptr, 1));      
        CHECK_NTHROW(OutOfGame<WagonCard>::moveCardTo(nullptr, nullptr, 1));              
