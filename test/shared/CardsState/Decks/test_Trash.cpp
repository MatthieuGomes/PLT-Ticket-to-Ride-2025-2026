...
#include "../../src/shared/cardsState/Trash.h"
...
TEST(Basic)
{
    ...
    }
    {
        ...
        Trash<DestinationCard> deck(cardsVec);
...
TEST(shuffle)
{
    ...
    }
    {
        ...
        std::vector<std::shared_ptr<WagonCard>> shuffledCardsVec = Trash<WagonCard>::shuffle(cardsVec);
...
{
    {
        ...
        WagonCard card2(cardArgs[1]);
        std::vector<std::shared_ptr<WagonCard>> cardsVec = std::vector<std::shared_ptr<WagonCard>>{std::make_shared<WagonCard>(card1), std::make_shared<WagonCard>(card2)};
        auto originTrash = std::make_shared<Trash<WagonCard>>(cardsVec);
        auto destinationTrash = std::make_shared<Trash<WagonCard>>();

        Trash<WagonCard>::moveCardTo(originTrash, destinationTrash, 0);

        CHECK_EQ(originTrash->countCards(), 1);
        CHECK_EQ(destinationTrash->countCards(), 1);

        CHECK_EQ(originTrash->cards[0]->color, ColorCard::BLUE);
        CHECK_EQ(destinationTrash->cards[0]->color, ColorCard::RED);
        ANN_END("moveCardTo")
    }
    {
        ...
        CHECK_NTHROW(Trash<WagonCard>::moveCardTo(nullptr, destinationTrash, 1)); 
        CHECK_NTHROW(Trash<WagonCard>::moveCardTo(originTrash, nullptr, 1));      
        CHECK_NTHROW(Trash<WagonCard>::moveCardTo(nullptr, nullptr, 1));          
