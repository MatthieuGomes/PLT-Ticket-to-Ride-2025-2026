...
#include "../../src/shared/cardsState/FaceUpCards.h"
...
TEST(Default)
{
    ...
    }
    {
        ...
        FaceUpCards<DestinationCard> deck;
        CHECK_EQ(deck.countCards(), 0);
        CHECK(deck.cards.empty());
...
{
    {
        ...
        FaceUpCards<WagonCard> deck(cardsVec);
    ...
    }
    {
        ...
        FaceUpCards<DestinationCard> deck(cardsVec);
    ...
    }
    {
        ...
        FaceUpCards<WagonCard> deck(emptyVec);
        CHECK_EQ(deck.countCards(), 0);
        CHECK(deck.cards.empty());
...
TEST(shuffle)
{
    ...
    }
    {
        ...
        std::vector<std::shared_ptr<WagonCard>> shuffledCardsVec = FaceUpCards<WagonCard>::shuffle(cardsVec);
...
{
    {
        ...
        WagonCard card2(cardArgs[1]);
        std::vector<std::shared_ptr<WagonCard>> cardsVec = std::vector<std::shared_ptr<WagonCard>>{std::make_shared<WagonCard>(card1), std::make_shared<WagonCard>(card2)};
        auto originFaceUpCards = std::make_shared<FaceUpCards<WagonCard>>(cardsVec);
        auto destinationFaceUpCards = std::make_shared<FaceUpCards<WagonCard>>();

        FaceUpCards<WagonCard>::moveCardTo(originFaceUpCards, destinationFaceUpCards, 0);

        CHECK_EQ(originFaceUpCards->countCards(), 1);
        CHECK_EQ(destinationFaceUpCards->countCards(), 1);

        CHECK_EQ(originFaceUpCards->cards[0]->color, ColorCard::BLUE);
        CHECK_EQ(destinationFaceUpCards->cards[0]->color, ColorCard::RED);
        ANN_END("moveCardTo")
    }
    {
        ...
        CHECK_NTHROW(FaceUpCards<WagonCard>::moveCardTo(nullptr, destinationFaceUpCards, 1)); 
        CHECK_NTHROW(FaceUpCards<WagonCard>::moveCardTo(originFaceUpCards, nullptr, 1));      
        CHECK_NTHROW(FaceUpCards<WagonCard>::moveCardTo(nullptr, nullptr, 1));         
