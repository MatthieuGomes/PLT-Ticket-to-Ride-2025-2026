...
std::vector<Station*> Map::listStations() const {
    return Stations;
}


std::vector<Road*> Map::listRoads() const {
    return Roads;
}

bool Map::claimRoad(Player* player, StationData* src, StationData* dest) {
    for (auto* r : Roads) {
        auto ep = r->data->endpoints;
        if ((ep[0] == src && ep[1] == dest) || (ep[0] == dest && ep[1] == src)) {
            if (r->data->owner == nullptr && !r->data->blocked) {
                r->data->owner = player;
                std::cout << "Road " << r->data->ID << " Was claimed.\n";
                return true;
            } else {
                std::cout << "Road already owned or blocked!\n";
                return false;
            }
        }
    }
...
Station* Map::getStationByName(const std::string& name) {
    for (auto* s : Stations)
        if (s->data->name == name)
            return s;
    return nullptr;
...
Road* Map::getRoad(Station* u, Station* v) {
    for (auto* r : Roads) {
        auto ep = r->data->endpoints;
...
Path Map::findShortestPath(StationData* src, StationData* dest) {
    Path path;
